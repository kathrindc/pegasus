...
export default class LogService {
    constructor(name) {
        ...
        if (!nameWidth || (typeof (nameWidth) === 'number' && nameWidth < name.length)) {
    ...
    error(text, err = null) {
        ...
        if (err && LogService.IsDebug) {
            let detail = `${err.stack || err}`;
...
LogService.LogLevels = LogLevels;
LogService.CurrentLogLevel = level;
LogService.IsDebug = level <= LogLevels.DEBUG;
